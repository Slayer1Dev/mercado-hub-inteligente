...
// Função de Autenticação (movida para cá)
export async function getValidAccessToken(supabase: any, userId: string): Promise<string> {
    ...
    const expirationTime = updatedAt + (expires_in * 1000) - (5 * 60 * 1000);
    ...
    const body = `grant_type=refresh_token&client_id=<span class="math-inline">\{ML\_CLIENT\_ID\}&client\_secret\=</span>{ML_CLIENT_SECRET}&refresh_token=${refresh_token}`;
